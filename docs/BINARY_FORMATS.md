...
| `4` | `uint32` | `version` | Format version: `3` |
...
- `Float64Array` of size $C_k \times 2$ containing `[lon, lat, lon, lat, ...]` coordinate pairs.

#### Ring Bounding Boxes (Version 2+)
- `Float64Array` of size $N_r \times 4$. Bounding boxes for each ring:
  `[minLon, minLat, maxLon, maxLat]` (used for outer/hole ring bounding-box early pruning).
