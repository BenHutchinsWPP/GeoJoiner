...
export interface KmlParseOptions {
  /** Property key for the <name> element. Default: "Name" */
  nameKey?: string;
  /** Property key for the <description> element. Default: "Description" */
...
export function parseKml(kmlText: string, options?: KmlParseOptions): KmlParseResult {
  const opts: Required<KmlParseOptions> = {
    nameKey: "Name",
  ...
  const propertiesSet = new Set<string>([opts.nameKey, opts.descriptionKey]);
  ...
  for (let i = 0; i < placemarks.length; i++) {
    ...
    const props = extractProperties(pm, opts);
    ...
    if (!geomEl) {
      warnings.push(`Placemark "${props[opts.nameKey] || i}" has no polygon geometry — skipping`);
      continue;
    }
    ...
    if (!coordinates) {
      warnings.push(`Placemark "${props[opts.nameKey] || i}" — could not extract valid polygon coordinates`);
      continue;
    }
...
function extractProperties(pm: Element, opts: Required<KmlParseOptions>): Record<string, string> {
  const props: Record<string, string> = {};

  // <name>
  const nameText = getChildText(pm, "name");
  if (nameText) props[opts.nameKey] = nameText.trim();

  // <description>
  const descText = getChildText(pm, "description");
  if (descText) props[opts.descriptionKey] = descText.trim();
